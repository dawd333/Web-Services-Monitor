...
class PingOverview extends React.Component {
      ...
      <Container className={styles.pingOverview}>
        ...
        >
          ...
          <VerticalRectSeries
            ...
            onValueMouseOver={this.valueOnMouseOver}
            onValueMouseOut={this.valueOnMouseOut}
          />
      ...
      ? results.map(result => {
          return {
            x0: convertFromUTCtoDateWithSecondsDifference(
              result.created_at,
              -this.props.pingModel.interval * 0.4
            ),
            ...
            y:
              result.rtt_avg_ms *
              (result.number_of_requests - result?.error_messages?.length)
          };
        })
      : [];
  };
  ...
  deletePingConfiguration = async () => {
    await this.props.deletePing(
      this.props.pingModel.service,
      this.props.pingModel.id
    );
